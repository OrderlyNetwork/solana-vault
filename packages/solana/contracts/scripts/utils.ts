...
export function getOAppConfigPda(OAPP_PROGRAM_ID: PublicKey): PublicKey {
    return PublicKey.findProgramAddressSync([Buffer.from(OAPP_SEED, 'utf8')], OAPP_PROGRAM_ID)[0]
}

export function getVaultOwnerPda(OAPP_PROGRAM_ID: PublicKey): PublicKey {
    return PublicKey.findProgramAddressSync([Buffer.from(constants.OWNER_SEED, 'utf8')], OAPP_PROGRAM_ID)[0]
}
...
export function getPendingInboundNoncePda(
    oappConfigPda: PublicKey,
    dstEid: number,
    peer_address: Uint8Array
): PublicKey {
    const bufferDstEid = Buffer.alloc(4)
    bufferDstEid.writeUInt32BE(dstEid)
    return PublicKey.findProgramAddressSync(
...
export async function createRelatedUSDCAcount(
    provider: anchor.AnchorProvider,
    wallet: anchor.Wallet,
    rpc: string,
    ENV: String
...
export function getOftConfigPda(OFT_PROGRAM_ID: PublicKey, mintAccount: PublicKey) {
    return PublicKey.findProgramAddressSync([Buffer.from(OFT_SEED, 'utf8'), mintAccount.toBuffer()], OFT_PROGRAM_ID)[0]
}

export function getVaultAuthorityPda(VAULT_PROGRAM_ID: PublicKey): PublicKey {
    return PublicKey.findProgramAddressSync([Buffer.from(constants.VAULT_AUTHORITY_SEED, 'utf8')], VAULT_PROGRAM_ID)[0]
}

async function sleep(number: number) {
    await new Promise((r) => setTimeout(r, number * 1000))
}
...
export function getManagerRolePda(
...
export function getSolVaultPda(VAULT_PROGRAM_ID: PublicKey): PublicKey {
    return PublicKey.findProgramAddressSync([Buffer.from(constants.SOL_VAULT_SEED, 'utf8')], VAULT_PROGRAM_ID)[0]
}
...
export function getManagerRolePdaWithBuf(
    VAULT_PROGRAM_ID: PublicKey,
    roleHash: number[],
    managerAddress: PublicKey
): PublicKey {
...
export function getUSDCAccount(usdc: PublicKey, owner: PublicKey): PublicKey {
    const usdcTokenAccount = getAssociatedTokenAddressSync(usdc, owner, true)
...
export function getSPLTokenAccount(splToken: PublicKey, owner: PublicKey): PublicKey {
    const splTokenAccount = getAssociatedTokenAddressSync(splToken, owner, true)
...
export function getTokenList(): string[] {
    return constants.TOKEN_SYMBOLS
}
