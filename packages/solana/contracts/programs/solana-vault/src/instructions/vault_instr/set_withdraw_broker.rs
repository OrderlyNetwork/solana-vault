...
use crate::state::{ManagerRole, WithdrawBroker};
...
impl SetWithdrawBroker<'_> {
    pub fn apply(
        ctx: &mut Context<SetWithdrawBroker>,
        params: &SetWithdrawBrokerParams,
    ) -> Result<()> {
        ...
        if params.allowed {
            msg!(
                "Setting withdraw broker index {:?}, broker hash {:?}",
                params.broker_index,
                broker_hash_hex
            );
            ...
            msg!(
                "Resetting withdraw broker index {:?}, broker hash {:?}",
                params.broker_index,
                broker_hash_hex
            );
